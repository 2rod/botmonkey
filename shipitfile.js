module.exports = function (shipit) {
	...
	var currentPath = config.DEPLOY_PATH + '/current';

	shipit.initConfig({
		default: {
			workspace: 'tmp',
			deployTo: config.DEPLOY_PATH,
		...
		production: {
			servers: config.DEPLOY_USERNAME + '@' + config.DEPLOY_HOSTNAME
		}
	});
	...
	shipit.blTask('start_screen', function () {
		return shipit.remote(pathStr + ' && cd ' + currentPath + ' && screen -S ' + config.DEPLOY_SCREEN + ' -d -m npm start');
	});
	...
	shipit.blTask('install_remote_config', function () {
		return shipit.remote('cd ' + config.DEPLOY_PATH + ' && cp config.json ' + currentPath);
	});
	...
	shipit.blTask('kill_screen', function () {
		return shipit.remote('if screen -ls | grep -q "' + config.DEPLOY_SCREEN + '"; then screen -S ' + config.DEPLOY_SCREEN + ' -p 0 -X stuff $'\\003'; fi;');
	});
